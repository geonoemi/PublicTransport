        ...
        try {
            for(i=0;i<DayTypes.getDayTypes().size();i++) {
                ...
                for( j=0; j<Station.getStationNames71A().size();j++) {
                    ...
                    if(!(Station.getStationNames71A().get(j).equals("Napfényfürdő"))) {

                        FileWriter file71Ab = new FileWriter("departure times\\"+"71A "+Station.getStationNames71A().get(j)+" "+DayTypes.getDayTypes().get(i)+" Napfényfürdő.txt");
